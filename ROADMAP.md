...
| **M23** ✅ (a–b) | Yarış hafta sonu | **(a–b landed; ADR-0031)** **M23a canlı zamanlama kulesi = replay:** kabuk-içi `RaceWeekend` ekranı son koşulan turun deterministik `RaceResult` telemetrisini tur-tur oynatır (pozisyon / gap / interval / lastik+yaş / pit + tur-filtreli olay feed'i + final klasman; oynat/duraklat/ileri-geri/başa-sona/hız) — **sıfır sim değişikliği, golden dokunulmaz** (replay'de RNG yok). **M23b oyuncu strateji kontrolü:** sıradaki tur için pilot-başına **başlangıç-bileşimi** seçimi (`RaceStrategy` + `Carset.PlayerRaceStrategies` → `RaceSimulator.Run` opsiyonel per-car `startingCompounds`; **opt-in-varsayılan → boşta/null byte-özdeş + golden korunur**; carset-taşımalı → reconstruction-tutarlı, sezon sınırında temizlenir; `CareerState` additive persist) + **Start Race** (üst-bar Continue'yu tetikler). Golden + byte-özdeşlik korundu. **Ertelendi:** şematik pist haritası + hareketli araç işaretçileri (mockup SVG), sektör renkleri/delta, telsiz mesajları, mid-race canlı komutlar (stepping-seam → golden riski), post-race rapor (= M24). |
