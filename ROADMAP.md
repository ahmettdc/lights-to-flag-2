...
| **M22** ⬛ | Yönetim ekranları | Finans, Ar-Ge, personel, tesisler, sözleşmeler (Takım Patronu). **Banka & kredi arayüzü** (kredi çek + o anki teklif oranı/taksit önizleme + açık borç/geri-ödeme planı + kredi puanı/tier + icra uyarıları) — **motor indi (Faz-A ✅):** deterministik `CreditProfile`/`BankLedger`/`BankEnforcement`; ödenmezse kademeli icra → varlık satışı → **ağır ceza/idari puan silme ama kovulma YOK** (Rev 15; ADR-0029). **Regülasyon & Uyum ekranı** (ADR-0010): aktif kurallar, kendi uyum/risk durumun, bekleyen değişiklikler, `Readiness` + Patron modunda **oylama** arayüzü. |
