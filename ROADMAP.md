...
> **Durum:** Faz 0 ✅ (M0–M2) · **Faz 1: M3 ✅ · M4 ✅ · M5 ✅ (a–e) · M6 ✅** (RNG + tur zamanı; lastik/yakıt/hava; yarış motoru: güvenilirlik + pilot hataları + çarpışma + start + nötralizasyon + zengin telemetri; **trafik: kirli hava / slipstream-DRS / geçiş mücadelesi**, CI yeşil) · **2026 regülasyon çağı ✅** (26a–c: aktif aero X/Z + Manuel Override + enerji/de-rating; carset `regulations` bloğuyla seçilebilir — ADR-0018) · **M7 ✅** (a–c: pit stop süre dağılımı + bileşim rotasyonu; kademeli pit pencereleri + güvenlik aracı altında ucuz stop; on-track cezalar: pist limitleri + güvensiz bırakma) · **M8 ✅** (sıralama Q1/Q2/Q3 + tek-tur/tek-seans → grid; antrenman programları → hafta sonu getirisi) · **M9 ✅** (a–g race-level: RaceFormat demeti; süreli/kısa yarış; pole/lider-tur/en-çok-lider puanları; başarı balastı; çok-sınıf klasman; rolling start + unlap kuralı; sprint puan tablosu; ters-grid/grid-ceza helper) · **M10 ✅** (altın-dosya determinizm hash'i — Win/macOS/Linux birebir; `ltf sweep` denge aracı) · **Faz 1 tamamlandı (M3–M10 + 2026 çağı).** · **Faz 2: M11 ✅ · M12 ✅** (M11 a–e: şampiyona + sezon motoru + takvim/Continue + kalıcı rekorlar + JSON kayıt/yükleme; M12 a–f: ilişki grafiği doğuşu + sözleşme yaşam döngüsü + pazarlık ilişkiyi okur + persistence — ADR-0013; **M13 a–g:** gelir/gider döngüsü + bütçe tavanı yaptırımı + finans persistence + `EconomySweep` çok-sezon denge süpürmesi — ekonomi kendini dengeliyor; **M14 a–n:** hibrit tech tree (4 departman/16 düğüm) + tam 10-tesis modeli + personel + **pist-doğrulama döngüsü** (ADR-0024) + **lastik aşınma fiziği** (nazik araç daha az aşınır → daha hızlı tur) + `ResearchSweep` çok-sezon süpürmesi — araç ölçülebilir/sınırlı/deterministik gelişir; ADR-0016/0020/0024; **M15 a–o:** tur-arası sezon dikişi (byte-özdeş) + bileşen kotası → grid cezaları + kalite-kontrol → güvenilirlik + takvim test günleri + **sezon-içi R&D dağıtımı** (yükseltme aynı sezonun sonraki turlarında hissedilir) — ADR-0011/0016/0020; **M17 a–l Core (Takım Patronu):** oyuncu-takımı + yönetim kurulu/baskı/kovulma riski (ADR-0025) + karar ledger'ları (sözleşme imzalama + R&D konsept/bütçe directive + regülasyon oyu) + cost-cap puan silme (ADR-0010) + headless deterministik `BossCareerSweep`, byte-özdeş-varsayılan; **M18 a–j Core (sezon devri):** sürücü potansiyel + yaşlanma + gelişim/gerileme + emeklilik + regen (kurgusal rookie) + transfer penceresi (`Team.DriverIds` yeniden yazılır) + regülasyon değişikliği uygulaması/hazırlıksız-drop-back + headless deterministik `WorldSweep`, byte-özdeş-varsayılan) · **Faz 2 tamamlandı (M11–M15 + M17–M18; M16 rafta).** · **Faz 3 başladı — M19 ✅ (a–o):** Avalonia kabuk — üst bar / sidebar / durum çubuğu + içerik bölgesi + **bildirim merkezi** + marka teması (palet + 3 OFL font + logo/pencere ikonu) + tasarım-sistemi kontrolleri (kart / rozet / progress / tooltip / buton / boş-yükleniyor-hata / toast / onay penceresi) + navigasyon + **flagship carset'ten canlı üst-bar** (Tarih / Cap Room / Board Conf / takım) + yerelleştirme altyapısı + **mimari gardiyan** (motor UI'sız); headless `[AvaloniaFact]` üç platform — ADR-0026; Continue inert (M21), ekran gövdeleri M20–M25 · **M20 ✅ (a–h):** ana menü + kök menü↔kabuk geçişi + yeni Takım Patronu kariyeri (carset + takım + **etkileşimli yönetim kurulu müzakeresi**) + kayıt/yükleme (slot + Devam) + ayarlar (kalıcı) + **Quick Race** + kabuktan Ayarlar/Menüye-Çık; zorluk saklanır ama motora bağlanmaz — ADR-0027 · **M21 ✅ (a–g):** beş kabuk-içi ekran (Paddock Hub / Drivers / Standings / Calendar / Database) + **canlı Continue** (public per-round `SeasonSimulator.RunRound` byte-özdeş çıkarım + değişebilir `LiveCareer`; puan durumu `(SeasonStart, seed, Date)`'ten yeniden kurulur, kayıt formatı değişmez) + **tarihli gelen kutusu** (`CareerNotificationSource`/`CareerNews`; eylem-gerektiren öğe Continue'yu durdurur — Rev 15) + **sezon-devri** (sezon sonunda M18 world-sweep zinciri; kariyer sonsuz) — golden + byte-özdeşlik korunur, ADR-0028 · **Banka & kredi motoru ✅** (Faz-A: dünyadan-türetilen `CreditProfile` puan/faiz/limit + `BankLedger` + kademeli icra — imzada-donan faiz, kovulma yok; `ltf sweep` banka bloğu; ADR-0029) · **M22 ✅ (a–f):** beş yönetim ekranı (Finance / R&D+Facilities / Cars+PowerUnit / Staff / Board+Sponsors) + **canlı ekonomi/banka/icra/board settle** (`LiveCareer.RollToNextSeason`) + **kredi çekme** arayüzü (Banka Faz-B, ilk oyuncu-mutasyonu) + **staff hire/release** + `CareerState` personel yakalaması; golden + byte-özdeşlik korundu, ADR-0028/0029 · **Canlı sezon-içi R&D ✅ (Model B, Ri1–Ri5):** araç sezon-**içinde** tur-tur gelişir (`LiveCareer` `RndProgression` ipler; canlı = reconstruct = `RunProgressed`, kayıt formatı değişmez — evrilen araç deterministik olduğundan persist değil reconstruct edilir) + R&D ekranından **konsept steer** + **FIA gelişim-dondurma rejimi** (üç-mod eksen dondurması: Full / InSeasonOnly [kış-darbesi] / Open + regülasyon-ballot ile dinamik açılıp-kapanma + persistence + R&D ekranı rozetleri; ADR-0030) — golden + byte-özdeşlik korundu · **M23 ✅ (a–j):** yarış hafta sonu — **canlı zamanlama kulesi = deterministik yarışın replay'i** (kabuk-içi `RaceWeekend`: son turun `RaceResult` telemetrisi tur-tur; pozisyon/gap/lastik/pit + olay feed'i + final klasman + oynat/duraklat/hız/atla; sıfır sim değişikliği, golden dokunulmaz) + **oyuncu başlangıç-bileşimi** (opsiyonel per-car `startingCompounds`, opt-in-varsayılan → boşta byte-özdeş) + **sekmeli Race/Qualifying** (quali reconstruct edilir, diske yazılmaz) + **şematik pist haritası + hareketli işaretçiler** (zaman-farkı yaklaşıklaması, tek generic outline — saf replay projeksiyonu) + **sektör renkleri/delta + telsiz feed'i** + **yarış motoru stepping-seam refactor** (`RaceStepper` tek implementasyon → byte-özdeş, parity-gated) + **gerçek yarış-içi canlı komutlar** (`RaceCommand` günlüğü + interaktif stepper: **RACE LIVE** → BOX/PUSH/EXTEND/MANAGE, bitişte commit → otorite `RunRound(log)`; **boş log → golden birebir; live = reconstruct = rollover**) — golden + byte-özdeşlik korundu (757 test yeşil), ADR-0031/0032 · **M24 ✅ (a–h):** istatistik & rekorlar — **kalıcı sezon arşivi** (`SeasonRecord`/`TrackRecord` + `Carset.SeasonHistory`/`TrackRecords`, inert-default → boş byte-özdeş; `SeasonArchive.Append` rollover'da, **append-only/birikir/reconstruct-değil-persist**; `CareerState` additive) + **RECORDS ekranı** (PROFILES / THIS SEASON / ALL-TIME / HALL OF FAME) + **kariyer profili zenginleştirme** (Drivers/Database'e de yansır) + **elde-çizili Canvas grafikleri** (`ChartScale`; sezon-içi puan ilerlemesi + çok-sezon kariyer trendi; VM path-string üretir → platform-bağımsız/testlenebilir) — sim'e sıfır dokunuş → golden + byte-özdeşlik korundu (784 test yeşil), ADR-0033 · Sıradaki: **M25** (öğretici) · Marka kiti + UI mockup teslim alındı (`design/`)
> · **Belge tarihi:** 2026-08-07 · **Belge dili:** Türkçe · **Oyun arayüz dili:** İngilizce
...
| Regülasyon | Statik | **Canlı FIA katmanı:** sezon-içi ihlal→ceza, sezon-sonu kural değişikliği→hazırlıksız geriler, **gelişim dondurma** (üç-mod eksen dondurması, ballot ile dinamik — ADR-0030) |
...
> **Banka & kredi sistemi (kullanıcı isteği) — Faz-A motoru ✅ · arayüz M22 — kredi + faiz + icra:**
> Takım faizle **kredi** çekebilir (banka/finansör; nakit-akışı kalemi, cost-cap harcaması değil).
> **Üçü de dünyayla paralel, deterministik, RNG yok — ve asla persist edilmez, dünyadan türetilir:**
> **(1) Kredi puanı** `CreditProfile` (0–100) beş sinyalin harmanı — şampiyona konumu, nakit ödeme gücü,
> `Car.Overall`, board güveni/kovulma-riski (yoksa düşer), geri-ödeme geçmişi. **(2) Faiz** = baz +
> puana bağlı risk primi — **teklif dinamik ama imza atılınca kredinin oranı ömrü boyunca donar**
> (kullanıcı kararı: "imzadan önce değişken, imzadan sonra sabit"). **(3) Limit** = hizmet-edilebilir
> gelirin katı × puan çarpanı; boş alan = limit − borç. Faiz + taksitler sezon kapanışında işler
> (`BankLedger.SettleSeason`); anapara ödendikçe **açık bakiye** düşer, karşılanamayan taksit **kaçırılan
> ödeme** olarak kapitalize olur. **İcra (ödenmezse) — kademeli, "ağır ceza · devam" (kullanıcı kararı):**
> **(1)** kaçırma → board baskısı + tarihli gelen-kutusu uyarısı (**Rev 15 eylem-gerektiren** → Continue
> durur); **(2)** tekrar → **zorunlu varlık satışı** (en yüksek maaşlı personeli nakde çevirir, borcu öder)
> + board darbesi; **(3)** sürdürülemez borç → **bir kereye mahsus idari puan silme** (`ConstructorPenalties`)
> + ağır tasfiye — **kovulma/oyun-sonu YOK, kariyer devam eder** (`FiringRisk`'e dokunulmaz). Saf aritmetik;
> `RaceSimulator`/altın hash'e değmez. `Loan` `Team.Finances`'te; carset `rules.bank` bloğu opsiyonel,
> varsayılan inert → mevcut carsetler **byte-özdeş**; krediler `CareerState`'te otomatik round-trip
> (M13f/M18h/M21f kalıbı). **İnen motor (Career):** `CreditProfile` + `BankLedger` + `BankEnforcement`
> + `BankSweep` (`ltf sweep` banka bloğu). **Arayüz (Faz-B):** **M22 Finance** (kredi çek + o anki teklif
> oranı/taksit önizleme + açık borç/geri-ödeme planı + kredi puanı/tier + icra uyarıları); M21 bildirim
> merkezine besler. Detay tasarımı: `docs/adr/0029`.
...
| **M15** ✅ | Test ve bileşen yönetimi | **GENİŞ M15**: tur-arası sezon dikişi + bileşen kotası/grid cezaları + kalite-kontrol→güvenilirlik + test günleri + sezon-içi R&D dağıtımı, deterministik, byte-özdeş-varsayılan. Alt-adımlar: **M15a** (`SeasonSimulator.RunProgressed` + `IBetweenRounds` — eski `Run` byte-özdeş kalır, tur-arası dikiş açar), **M15b–d** (`ComponentPenalties` saf fonksiyon: pilot-başı bileşen havuzu, tam-oturur ömür varsayılanı → sıfır ceza/byte-özdeş; `ComponentReliabilityWearInfluence` opt-in → az güvenilir araç kotayı aşar → hazır `GridOrder.WithPenalties` ile grid cezası; `ComponentLifeRounds` kolu), **M15e–f** (kalite-kontrol tesisi → güvenilirlik: `SeasonEntries.Build` reliability bump, `QualityControlReliabilityInfluence` varsayılan 0 → altın-hash korunur; matematik Career'da), **M15g** (`ResearchLedger.DevelopStep` eklemeli tur-dilimi; `DevelopSeason` dokunulmaz), **M15h** (`TestDay` + `Carset.TestDays` + `CalendarEventKind.TestDay` üretimi), **M15i–j** (`RndProgression`: tur-başı geliştirme + test-günü pulse; `ResearchSweep` `RunProgressed`'a geçer — sezon ortasında onaylanan yükseltme **aynı sezonun sonraki turlarında** hissedilir), **M15k** (persistence: sezon-ortası durum M14 `TeamResearchRecord`'da byte-özdeş round-trip; yeni kayıt yok), **M15l–n** (flagship opt-in: bileşen/QC/test-günü içeriği + `FlagshipComponentTests` + `ltf sweep` bileşen özeti). **Ertelenenler:** bileşen havuzunun mid-season kalıcılaştırılması, M5b mileage bağı, homologasyon (ADR-0019/M22), tesis yatırım/bakım maliyeti (M13/ADR-0020), pilot antrenman test-günleri (ADR-0015), interaktif runner (Faz-3 UI), regülasyon budaması (M18). |
...
| **M17** ✅ | **Takım Patronu modu** (tek kariyer modu) | **Core M17**: oyuncu-takımı + yönetim kurulu/baskı/kovulma riski + karar ledger'ları + regülasyon oyu, headless deterministik kariyer sweep'i (`BossCareerSweep`), byte-özdeş-varsayılan, kayıt korur. Alt-adımlar: **M17a** (`Carset.PlayerTeamId` + resolver — `""` = tüm-AI, `SeasonSimulator` okumaz), **M17b** (kurul/sahiplik/baskı domaini: `OwnershipType` 6-tip, `BoardMember` öncelik+trait+`ConfidenceInPlayer`, `PressureMetrics` 6×0–100, `Objective`+`ObjectiveVisibility`, `TeamBoard` + `Carset.Boards=[]`; yeni `Pressure` 0–100 struct), **M17c** (content loader + validator — `playerTeamId`/`boards`/`regulationProposals`, bloksuz carset byte-özdeş), **M17d** (`ContractSigning.Sign` — `ContractNegotiation.Evaluate` geçitli kabul→`Contracts`'a ekle, ret→no-op), **M17e** (`ResearchLedger`'a opsiyonel `IDevelopmentDirectives` dikişi — `RndDirection` konsept+bütçe; null/nötr byte-özdeş → `ResearchSweep` değerleri korunur), **M17f** (cost-cap sonuçları: R&D harcaması cap'e katlanır ama bakiyeye yansımaz + `ConstructorPenalties.Apply` puan silme → yeniden sırala), **M17g** (`RegulationProposal`+`RegulationVote` + `RegulationBallot.Resolve` — heuristik oy + oyuncu override, pass/fail kaydeder; kural/era dokunulmaz), **M17h** (`IBossPolicy`/`BossPolicy` + `BoardReview` SetObjectives/Assess/firing — saf öncelik-ağırlıklı delta, RNG yalnız FNV salt(takım) tehlike bandında; `SeasonCalendar.ForCareer` `BoardReview` olayları), **M17i** (`BossCareerSweep` — SetObjectives→RunProgressed(directive)→SettleSeason(rndSpend)→ConstructorPenalties→BoardReview.Assess→imzalama/oy→rollover; boardless sweep `ResearchSweep` araç yörüngesini birebir üretir), **M17j** (persistence — `CareerState.PlayerTeamId`+`Boards` plain-int `TeamBoardRecord`, capture-when-non-empty, byte-özdeş round-trip), **M17k** (flagship opt-in: Talon Racing oyuncu-takımı + 3-üyeli manufacturer-backed kurul + hedefler + kurgusal `regulationProposals`; `ltf sweep` Team-Principal özeti; `FlagshipBossTests` — kurul evrilir + motor byte-özdeş). **Ertelenenler:** hukuk/scout/akademi/favoritizm/medya-sistemi, tam 10-takım yaşayan kurul (M18/ADR-0022), **regülasyon kural-değişikliği uygulaması + hazırlıksız-ceza (M18 — M17 yalnız oyu kaydeder)**, interaktif Continue-runner + günlük tick (Faz-3 UI, ADR-0011), transfer/regen/yaşlanma (M18). |
| **M18** ✅ | Sezon devri | **Core M18**: sürücü yaşam döngüsü (potansiyel + yaşlanma + gelişim/gerileme eğrisi + emeklilik) + regen + transfer penceresi + regülasyon değişikliği uygulaması/drop-back, headless çok-sezon `WorldSweep`, byte-özdeş-varsayılan. Alt-adımlar: **M18a** (`Driver.Potential` gizli tavan, düz `int` 0-sentinel), **M18b** (`DriverDevelopmentRules` yaş-eğrisi tuning + `RegulationUnreadinessPenalty`, hepsi 0 → `IsActive=false` inert; loader/validator), **M18c** (`DriverProgression.Advance` — yaşlanma + potansiyele büyüme + peak-sonrası gerileme [fiziksel hızlı: Pace/Consistency; deneyim yavaş], `Fork(Salt(id))`, `!IsActive → Assert.Same`), **M18d** (`DriverRetirement.Retire` — atıl `RetirementAge`'i tüket, koltuğu boşalt), **M18e** (`RookieGenerator.Generate` — deterministik kurgusal rookie, in-code isim tabloları, ADR-0003), **M18f** (`TransferMarket.Resolve` — **`Team.DriverIds`'i yeniden yazan eksik yazıcı**: worst-first standings, serbest-ajan → `Reserves` → regen, `ExpectedSalary`'de imzala; 0 boşluk → byte-özdeş), **M18g** (`RegulationChange.Apply` — geçen proposal'ı uygula + `(100−Readiness)/100 × Magnitude × coeff` ile hazırlıksız takımı geriye düş; ilk `RegulationReadiness` okuyucusu; coeff 0 / proposal yok → inert), **M18h** (persistence revizyonu — tam dinamik `DriverRosterRecord` + `Reserves` + `TeamRosterRecord` + regen kurtarma union-on-restore; dev-aktifken snapshot yetkili, değilse hafif delta → inert round-trip byte-özdeş), **M18i** (`WorldSweep.Run` — RelationshipEvolution [M12 atıl bonus] → CareerRollover → DriverProgression → DriverRetirement → ContractLedger → TransferMarket → RegulationChange; inert carset → hepsi no-op), **M18j** (flagship opt-in: `driverDevelopment` + kurgusal `reserves` havuzu + `regulationUnreadinessPenalty` + driver `potential` loader; `ltf sweep` World özeti; `FlagshipWorldTests` — grid yaşlanıp emekli edip yeniden dolar, deterministik/sınırlı; mevcut Economy/Research/Boss/Component flagship testleri motoru korur). **Ertelenenler:** yaşayan-dünya ekosistemleri (takım sahiplik/iflas, üretici giriş/çıkış, sponsor yaşam döngüsü — ADR-0022 Geniş), ilişki→olay feud + mentorluk (ADR-0013), haber/söylenti + tarih DB + hall-of-fame (M24), scout/scouted%/rapor (M14/M22), `CalendarEventKind.TransferWindow`/`.RegulationAnnouncement` takvim-yüzeyi (Faz-3 UI), interaktif Continue-runner. |
...
| **M22** ✅ (a–f) | Yönetim ekranları | **Beş yönetim ekranı indi** (Finance / R&D+Facilities / Cars+PowerUnit / Staff / Board+Sponsors), M21 ekran deseniyle — salt-okunur projeksiyon + iki oyuncu-mutasyon ailesi. **M22a canlı settle zinciri:** `LiveCareer.RollToNextSeason` artık sezon sınırında ekonomi→banka→icra→board settle koşuyor (world-sweep'ten önce), yani finans/board **canlı evriliyor**; icra eylem-gerektiren gelen-kutusu öğesi doğurup Continue'yu durduruyor (Rev 15). **M22b/c Finance + Banka Faz-B:** bakiye/gelir/cost-cap + kredi puanı/tier/oran/limit + açık borç/taksit + icra uyarıları; **kredi çekme** (`BankLedger.Borrow`, ilk oyuncu-mutasyonu, `ApplyToSeasonStart` deseniyle `SeasonStart`'a inip kayıtta yaşıyor). **M22f Staff:** personel + serbest havuz + **hire/release** (`StaffLedger`) + `CareerState` personel yakalaması (byte-özdeşlik-korumalı; icra tasfiyesini de kalıcı yapar). Golden + byte-özdeşlik korundu; ADR-0028 (rollover semantiği güncellendi) / ADR-0029 (Faz-B). **Ertelendi:** canlı R&D geliştirmesi (per-round seam ayrı milestone; R&D ekranı salt-okunur), bağımsız **Regülasyon & Uyum ekranı** (NavKey yok — readiness R&D/Board'a katlandı; oylama Patron akışında). |
| **M23** ✅ (a–j) | Yarış hafta sonu | **(a–j landed; ADR-0031/0032)** **M23a–c canlı zamanlama kulesi = replay** (kabuk-içi `RaceWeekend` son koşulan turun deterministik `RaceResult` telemetrisini tur-tur oynatır — pozisyon/gap/interval/lastik+yaş/pit + tur-filtreli olay feed'i + final klasman + oynat/duraklat/hız; **sıfır sim değişikliği, golden dokunulmaz**) + **oyuncu başlangıç-bileşimi** (`RaceStrategy` + `Carset.PlayerRaceStrategies` → `Run` opsiyonel per-car `startingCompounds`; **opt-in-varsayılan → boşta byte-özdeş**, reconstruction-tutarlı, `CareerState` additive persist) + docs. **M23d sekmeli ekran + QUALIFYING sekmesi** (`RoundOutcome.Qualifying` döndürülür; `LiveCareer._qualifying` paralel liste → `(SeasonStart, seed, Date)`'ten **reconstruct**, diske yazılmaz — grid/pilot/Part/BestLap/sektörler). **M23e şematik pist haritası** (`TrackOutline`: mockup Bézier → ön-örneklenmiş polyline + `PointAtFraction`; işaretçi konumu `GapToLeader ÷ referans-tur` yaklaşıklaması, takım-renkli/oyuncu-vurgulu — **saf replay projeksiyonu**, tek generic outline, per-circuit geometri yok). **M23f sektör renkleri/delta** (session-best mor / personal-best yeşil defteri) **+ telsiz feed'i** (olaydan türetilen zengin metin). **M23g yarış motoru stepping-seam refactor** (`RaceSimulator.Run`'ın lap-loop'u tek `RaceStepper`'a taşınır; `Run` stepper'ı sürer → **byte-özdeş** [tek implementasyon, drift yok]; parity-test + golden-hash kapı — projedeki en yüksek-riskli iş, izole/parity-gated). **M23h canlı komut kanalı** (`RaceCommand`{Round,Lap,DriverId,Kind} + `Carset.PlayerRaceCommands`; stepper opsiyonel komut-günlüğü alır, **saf-aritmetik / aynı-stream draw** ile uygular → **boş log byte-özdeş, golden birebir**; `CareerState` additive persist; ADR-0032). **M23i interaktif canlı yarış** (`RaceStepper.Issue` gelecek-lap emri enjekte eder = kurulum-günlüğüyle özdeş → **live = reconstruct**; `SeasonSimulator.StartRound` tek paylaşımlı kurulum; **RACE LIVE** + pit-duvarı **BOX/PUSH/EXTEND/MANAGE** butonları; bitişte log commit → otorite sonuç **`RunRound(log)`**; komut vermeden oyna → bugünkü yarış birebir). Golden + byte-özdeşlik korundu (757 test yeşil). **Ertelendi:** Practice sekmesi (kariyerde practice koşulmuyor → ayrı gameplay sistemi), post-race rapor/istatistik (= M24). |
| **M24** ✅ (a–h) | **İstatistik ve rekorlar** | **(a–h landed; ADR-0033)** Motor sezon sonuçlarını rollover'da skaler sayaçlara indirip atıyordu (geçmiş reconstruct edilemez) → yıl-yıl hall of fame / çok-sezon trend / pist rekoru için **yeni kalıcı arşiv**. **M24a–c kalıcı sezon arşivi:** `SeasonRecord`/`TrackRecord` domain tipleri + `Carset.SeasonHistory`/`TrackRecords` (inert-default → boş byte-özdeş); `SeasonArchive.Append` `LiveCareer.RollToNextSeason`'da (`CareerRollover.Apply`'dan sonra, oynanan takvimle) — **append-only, birikir, temizlenmez, reconstruct-değil-persist**; `CareerState` additive round-trip. **M24d RECORDS ekranı** (yeni `NavKey.Records`, 4-sekmeli `TabControl`) **+ PROFILES** (pilot kariyer leaderboard'u) **+ `EntityDetailViewModel` "CAREER RECORD" zenginleştirme** (Drivers/Database'e de yansır). **M24e ALL-TIME** (kümülatif top-5 leaderboard'lar) **+ HALL OF FAME** (yıl-yıl şampiyonlar + pist rekorları, boş-durum). **M24f THIS SEASON** (sezon liderleri + takım-eşi kafa-kafaya + elde-çizili kümülatif puan grafiği) **+ `ChartScale`** (M23 `TrackOutline` analoğu; VM path-string üretir → **platform-bağımsız/testlenebilir**, view parse eder). **M24g çok-sezon kariyer trendi** (seçili pilotun sezon-sezon puanı, PROFILES detayına gömülü). Golden + byte-özdeşlik korundu (784 test yeşil); sim'e sıfır dokunuş, sweep/parite dokunulmaz. **Ertelendi:** haber/söylenti motoru (ADR-0022), scout/rapor (M14/M22), Practice istatistikleri, grafik animasyonları. |
