...
| **M14** ⬛ | Ar-Ge ve personel | **Departman-bazlı hibrit tech tree** (ADR-0016 Rev 30 — Aerodinamik/Şasi/PU/Dayanıklılık departmanları; **Minor/Major/Ultimate** düğümler; düğüm aç → araç puanı + konsept yönü; kategoriler ADR-0024 12-eksenine eşlenir, front/rear-DF/DRS değil), kaynak dağıtımı ($ bütçe + CFD kotası + tesis slotu — Resource Points yok), tesis yükseltmeleri (rüzgâr tüneli, simülatör), personel işe alımı. Sezon içi gelişim **simülasyonda gerçekten hissedilir** (Faz 1'e bağlanır). **"Regülasyon hazırlığı" programı** (ADR-0010): yaklaşan kurallara `Readiness` için kaynak ayırma. İlerleme **takvime göre günlük/haftalık** işler (ADR-0011). |
